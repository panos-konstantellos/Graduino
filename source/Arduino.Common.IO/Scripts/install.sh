...
dotnet publish Arduino.Listener/Arduino.Listener.csproj -c Release --no-restore --no-build -o ./out/
...
if service_exists Graduino; then
    systemctl stop Graduino
fi

rm -rf /opt/Graduino
cp -rf ./out /opt/Graduino
cp -rf Scripts/Graduino.service /etc/systemd/system/Graduino.service

systemctl daemon-reload
systemctl start Graduino
